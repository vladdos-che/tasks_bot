...
import { Megaphone, Save, Play, RotateCcw, Plus, Trash2, GripVertical, Crown } from 'lucide-react';

export default function VpsConducting() {
  ...
  const fetchAll = async () => {
    try {
      const [qRes, cRes, uRes] = await Promise.all([
        axios.get('/api/vps-conducting/queue', { headers }),
        axios.get('/api/vps-conducting/config', { headers }),
  ...
  const addToQueue = async () => {
    ...
    try {
      const res = await axios.post('/api/vps-conducting/queue', { user_ids: newUserIds }, { headers });
      setQueue(res.data);
      setSelectedUserId('');
  ...
  const rotateQueue = async () => {
    try {
      const res = await axios.post('/api/vps-conducting/rotate', {}, { headers });
      setQueue(res.data);
  ...
  const handleDrop = async (e, dropIndex) => {
    ...
    try {
      const res = await axios.post('/api/vps-conducting/queue', {
  ...
  const saveConfig = async () => {
    setIsSaving(true);
    try {
      await axios.post('/api/vps-conducting/config', config, { headers });
  ...
  const forceSend = async () => {
    try {
      await axios.post('/api/vps-conducting/force-send', {}, { headers });
    ...
    <div style={{ maxWidth: '900px', margin: '0 auto' }}>
      <div className="flex justify-between items-center" style={{ marginBottom: '1.5rem' }}>
        <h1 style={{ margin: 0, display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
          <Megaphone size={28} /> Проведение ВПС
        </h1>
      </div>
      ...
      <div className="glass-panel" style={{ padding: '1.5rem' }}>
        ...
        <div className="flex gap-2 mb-4 flex-wrap">
          ...
          >
            <option value="">Выберите брата…</option>
        ...
        {queue.length === 0 ? (
          <div className="text-center text-secondary" style={{ padding: '3rem 1rem' }}>
            Очередь пуста. Добавьте братьев для ротации.
          </div>
        ) : (
          <div
